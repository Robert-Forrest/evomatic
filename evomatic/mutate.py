...
def mutate(alloys):
    ...
    for _, alloy in alloys.iterrows():
        if np.random.uniform() < evo.parameters['mutation_rate']:
            ...
            if(len(mutant_alloy_composition) < evo.parameters['constraints']['max_elements']):
                mutation_types.append('add')
            if(len(mutant_alloy_composition) > evo.parameters['constraints']['min_elements']):
            ...
            if mutation_type == 'remove':
                ...
                for element in evo.parameters['constraints']['percentages']:
                    if element in removable_elements:
                        if evo.parameters['constraints']['percentages'][element]['min'] > 0:
                ...
                element_to_adjust = np.random.choice(
                    list(mutant_alloy_composition.keys()), 1)[0]
    ...
    for i in range(len(mutantIndices)):
        alloys.at[
            int(mutantIndices[i]),'alloy'] = mutants[i]['alloy']
